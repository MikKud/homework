...
int main()
{
	...
	cout << "getdata - data of opened file" << endl;
	while (command != "end")
	{
		cin >> command;
		if (command == "getdata")
		{
			cout << Fileptr->getData() << endl;
		...
		{
			Uptr=Stptr;
			cout << "we now at the top directory" << endl;
		}
		if (command == "newfile")
		{
			int data;
			string name;
			cout << "Enter name and data" << endl;
			cin >> name >> data;
			NewFptr->reName(name);
		...
		if (command == "newdir")
		{
			...
			Uptr->addDir(*NewDptr);
			cout << "New Directory were created" << endl;
		}
		if (command == "ListALL")
		{
			Uptr->LSR(*Uptr);
